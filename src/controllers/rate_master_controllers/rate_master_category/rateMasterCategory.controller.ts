...
import * as categoryService from "./rateMasterCategory.service.js"; // TODO confirm path
...
export const initializeRateMasterCategories = async (
  ...
  try {
    ...
    const result = await categoryService.initializeRateMasterCategories(organizationId, userId);

    if (result.alreadyInitialized) {
      res.status(200).json({
        ok: true,
        data: result,
        message: "Rate master categories already initialized for this organization",
      });
      return;
    }
    ...
    res.status(201).json({
      ok: true,
      data: result,
      message: `Rate master categories initialized: ${result.categoriesCreated} categories`,
    });
  } catch (error) {
    next(error);
  }
